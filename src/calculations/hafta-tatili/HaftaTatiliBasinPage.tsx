...
export default function HaftaTatiliBasinPage() {
  ...
  const recalcRowCb = useCallback(
    (row: HaftaTatiliTableRow) => recalcRow(row, isGeceCalisan),
    [isGeceCalisan],
  );
  ...
  } = useHaftaTatiliManualBrutDisplay(haftaTatiliRows, recalcRowCb);
  ...
  const handleRowWeekChange = useCallback((i: number, val: string) => {
    ...
    setHaftaTatiliRows((prev) =>
      prev.map((r, idx) => (idx !== i ? r : recalcRow({ ...r, weekCount: wc, manualWeekCount: true }, isGeceCalisan))),
    );
  }, [isGeceCalisan]);

  const handleRowWageChange = useCallback(
    (i: number, val: string) => {
      ...
      setHaftaTatiliRows((prev) => {
        ...
        const next = prev.map((r, idx) => (idx !== i ? r : recalcRow({ ...r, wage }, isGeceCalisan)));
        if (rid) {
          queueMicrotask(() =>
            setRowOverrides((o) =>
  ...
  const wordSections = useMemo(() => {
    ...
    return sections;
  }, [dateRanges, haftaTatiliExpiryStart, haftaTatiliExcludedDays, displayHtRows, totalBrut, haftaTatiliNetSummary, isGeceCalisan]);
