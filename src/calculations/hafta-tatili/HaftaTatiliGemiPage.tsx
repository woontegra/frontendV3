/**
 * Gemi Adamı Hafta Tatili Alacağı Hesaplama
...
import { reduceRowOverridesWithManualBrut } from "@/utils/fazlaMesai/fmManualWageRowOverrides";
...
import { extractPeriodISO, withHtRowIds, newHtRowId } from "./haftaTatiliRowUtils";
...
const RECORD_TYPE = "hafta_tatili_gemi_adami";
const REDIRECT_BASE = "/hafta-tatili/gemi-adami";
...
export default function HaftaTatiliGemiPage() {
  ...
  const videoLink = getVideoLink("hafta-gemi");
  ...
  const handleRowDateChange = useCallback(
    ...
    [haftaTatiliExcludedDays, selectedHolidayIds]
  );
  ...
  const handleSave = () => {
    ...
    kaydetAc({
      ...
      varsayilanIsim: `Gemi Adamı Hafta Tatili - ${new Date().toLocaleDateString("tr-TR")}`,
  ...
  const handlePrint = () => {
    const el = document.getElementById("ht-gemi-report-content");
    if (!el) return;
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] w-full bg-gray-50 dark:bg-gray-900 pb-28">
            ...
            <div className="p-3 sm:p-4 space-y-5">
              ...
              <section>
                ...
                <div className="mt-1.5 rounded border border-gray-200 dark:border-gray-600 bg-gray-50/50 dark:bg-gray-900/30 p-2.5 text-[11px] font-light text-gray-500 dark:text-gray-400 space-y-0.5">
                  <p>854 Sayılı Deniz İş Kanunu'nun 40. maddesi gereğince gemi adamlarına hafta tatili ücreti ödenir.</p>
      ...
      <FooterActions
        ...
        previewButton={{
          title: PAGE_TITLE,
          copyTargetId: "ht-gemi-word-copy",
          hideWordDownload: true,
          renderContent: () => (
            <div>
              <style>{`
                .ht-gemi-sec { margin-bottom: 1.25rem; }
                .ht-gemi-sec .sec-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; }
                .ht-gemi-sec .sec-title { font-weight: 600; font-size: 0.75rem; color: #374151; }
                .ht-gemi-copy-btn { background: transparent; border: none; cursor: pointer; padding: 0.25rem; border-radius: 0.375rem; color: #6b7280; }
                .ht-gemi-copy-btn:hover { background: #f3f4f6; color: #374151; }
                #ht-gemi-word-copy .sec-content { border: none; overflow-x: auto; padding: 0; margin: 0; }
                #ht-gemi-word-copy table { border-collapse: collapse; width: 100%; margin: 0; font-size: 0.75rem; color: #111827; }
                #ht-gemi-word-copy td, #ht-gemi-word-copy th { border: 1px solid #999; padding: 5px 8px; background: #fff !important; color: #111827 !important; white-space: nowrap; }
                #ht-gemi-word-copy td:last-child, #ht-gemi-word-copy th:last-child { text-align: right; width: 38%; }
              `}</style>
              <div id="ht-gemi-word-copy">
                {wordSections.map((sec) => (
                  <div key={sec.id} className="ht-gemi-sec report-section" data-section={sec.id}>
                    <div className="sec-header">
                      <span className="sec-title">{sec.title}</span>
                      <button
                        type="button"
                        className="ht-gemi-copy-btn"
                        onClick={async () => {
          ...
          onPdf: () => downloadPdfFromDOM(PAGE_TITLE, "ht-gemi-report-content"),
        }}
      />

      <div style={{ display: "none" }}>
        <div id="ht-gemi-report-content" style={{ fontFamily: "Inter, Arial, sans-serif", color: "#111827", maxWidth: "16cm", padding: "0 12px" }}>
          <style>{`#ht-gemi-report-content table{width:100%;border-collapse:collapse;border:1px solid #999;margin-bottom:8px}#ht-gemi-report-content td,#ht-gemi-report-content th{border:1px solid #999;padding:5px 8px;font-size:10px}#ht-gemi-report-content h2{font-size:12px;font-weight:700;margin:10px 0 5px}`}</style>
