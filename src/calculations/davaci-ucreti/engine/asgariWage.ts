/**
 * Asgari ucret tablosu ve donem yardimcilari.
 * Yalnizca davaci-ucreti modulu icinde kullanilir.
 */

// Asgari Ãœcret Tablosu
...
/**
 * Bir yÄ±lda 2 dÃ¶nem asgari Ã¼cret olup olmadÄ±ÄŸÄ±nÄ± kontrol et
 * @param year - YÄ±l
 * @returns true ise 2 dÃ¶nem var, false ise tek dÃ¶nem
 */
...
/**
 * YÄ±l ve dÃ¶neme gÃ¶re asgari Ã¼creti getir
 * @param year - YÄ±l
 * @param period - 1: Ocak-Haziran, 2: Temmuz-AralÄ±k
 * @returns Asgari Ã¼cret veya null
 */
export function getAsgariUcretByYearAndPeriod(year: number, period: 1 | 2): number | null {
  if (period === 1) {
    // 1. DÃ¶nem: Ocak-Haziran (01.01 - 30.06)
    const checkDate = `${year}-03-01`; // DÃ¶nemin ortasÄ± (Mart)
    return getAsgariUcretByDate(checkDate);
  } else {
    // 2. DÃ¶nem: Temmuz-AralÄ±k (01.07 - 31.12)
    const checkDate = `${year}-09-01`; // DÃ¶nemin ortasÄ± (EylÃ¼l)
    return getAsgariUcretByDate(checkDate);
  }
}
