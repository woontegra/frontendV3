/**
 * Kısmi süreli / part time — yıllık ücretli izin (v1 KismiIndependent ile uyumlu, v2 shell).
...
const NOTE_ITEMS: string[] = [
  "Kısmi süreli çalışanlarda yıllık izin hakkı tam süreli çalışanlara göre orantılı olarak hesaplanır.",
  "Haftalık çalışma süresi 45 saatten az olan işçiler kısmi süreli sayılır.",
];
...
const REPORT_CONTENT_ID = "report-content-yillik-kismi";
...
export default function YillikIzinKismiPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const response = await apiClient("/api/yillik-izin/kismi", {
          method: "POST",
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (kismiReportConfig.infoRows?.length) {
    ...
    return sections;
  }, [kismiReportConfig, workPeriods, breakdown, usedTotal, remainingDays, brutUcret, brutIzin, rows]);
    ...
    <div className="min-h-[calc(100vh-4rem)] w-full bg-gray-50 dark:bg-gray-900 pb-28">
      ...
      <FooterActions
        ...
        previewButton={{
          ...
          copyTargetId: "yillik-kismi-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #yillik-kismi-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #yillik-kismi-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="yillik-kismi-word-copy">
          ...
          onPdf: () => downloadPdfFromDOM(`Kısmi Süreli ${REPORT_TITLE} Rapor`, REPORT_CONTENT_ID),
        }}
      />
