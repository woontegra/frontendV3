/**
 * Borçlar Kanunu — yıllık ücretli izin (v1 BorclarKanunuIndependent ile uyumlu).
 */
...
const NOTE_ITEMS: string[] = [
  "Borçlar Kanununa tabi iş ilişkilerinde yıllık ücretli izin hakkı yılda en az 4 haftadır.",
  "Bu hesaplama Borçlar Kanunu kapsamında çalışanlar için yapılmıştır.",
];
...
const REPORT_CONTENT_ID = "report-content-yillik-borclar";
...
export default function YillikIzinBorclarPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const response = await apiClient("/api/yillik-izin/borclar-kanunu", {
          method: "POST",
        ...
        console.error("Yıllık izin (Borçlar) hesaplama hatası:", err);
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (reportConfig.infoRows?.length) {
      const n1 = adaptToWordTable({
        headers: ["Alan", "Değer"],
        rows: reportConfig.infoRows.map((r) => [r.label, String(r.value ?? "-")]),
      });
    ...
    const md = reportConfig.mahsuplasmaData;
    if (md?.rows) {
    ...
    return sections;
  }, [reportConfig, rows, diff, is18Or50, breakdown, usedTotal, remainingDays, brutUcret, brutIzin, employerPayment]);
    ...
    <div className="min-h-[calc(100vh-4rem)] w-full bg-gray-50 dark:bg-gray-900 pb-28">
      ...
      <div className="w-full py-3 sm:py-4 px-3 sm:px-4">
          ...
          <div className="p-4 sm:p-6 space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input disabled value={diff.label} className={`${inputCls} mt-1 bg-gray-50 dark:bg-gray-900/40`} />
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-end">
              <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-gray-700 dark:text-gray-300">
                ...
                />
                18 yaş altı / 50 yaş üstü (3 hafta)
              </label>
            </div>

            <div className="p-4 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/25 dark:to-cyan-950/20 border-l-4 border-blue-500">
              <div className="text-base font-bold text-gray-900 dark:text-white text-center flex flex-wrap items-center justify-center gap-2">
                {is18Or50 ? (
                  <>
              ...
              <p className="text-center text-xs text-gray-600 dark:text-gray-400 mt-2">Toplam izin hakkı (kullanılan düşülmeden önce)</p>
            </div>
        ...
        <div className="mt-6 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 sm:p-5 shadow-sm">
          ...
          <ul className="space-y-2 text-[11px] font-light text-gray-500 dark:text-gray-400 list-disc pl-4">
