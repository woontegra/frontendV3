/**
 * Belirli süreli iş sözleşmesi — yıllık ücretli izin (v1 BelirliIndependent ile uyumlu, v2 shell).
 * Çoklu çalışma dönemi kaydı tutulur; hizmet süresi ilk dönemin girişi ile son dönemin çıkışına göre hesaplanır (v1 ile aynı).
 */
...
const NOTE_ITEMS: string[] = [
  "Belirli süreli iş sözleşmelerinde yıllık ücretli izin hakkı, fiilen çalışılan süreye ve İş Kanunu’ndaki genel hükümlere göre değerlendirilir.",
  "Sözleşme süresi ile fiili çalışma dönemleri farklıysa tarihleri ayrı dönemler olarak girebilirsiniz.",
];
...
const REPORT_CONTENT_ID = "report-content-yillik-belirli";
...
export default function YillikIzinBelirliPage() {
  ...
  const addWorkPeriod = () => {
    setWorkPeriods((prev) => [...prev, { id: Math.random().toString(36).slice(2), iseGiris: "", istenCikis: "" }]);
  };
  const removeWorkPeriod = (pid: string) => {
    if (workPeriods.length > 1) setWorkPeriods((prev) => prev.filter((p) => p.id !== pid));
  };
  ...
  const diff = useMemo(() => {
    ...
    return { yil: wp.years, label: wp.label };
  }, [iseGiris, istenCikis]);
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const response = await apiClient("/api/yillik-izin/belirli", {
          method: "POST",
    ...
    yukleHesap(effectiveId, RECORD_TYPE)
      .then((res) => {
        ...
        if (Array.isArray(form?.workPeriods) && form.workPeriods.length > 0) {
          ...
          const g = form?.iseGiris || form?.ise_giris;
          const c = form?.istenCikis || form?.isten_cikis;
          if (g || c) {
            setWorkPeriods([{ id: "1", iseGiris: g || "", istenCikis: c || "" }]);
          }
        }
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={tdRightStyle}>{remainingDays} gün</td>
                </tr>
                <tr>
                ...
                <tr style={{ fontWeight: 600, backgroundColor: "#f3f4f6" }}>
                  ...
                  <td style={tdRightStyle}>{fmtLocal(brutIzin)}₺</td>
                </tr>
              </tbody>
            </table>
          ),
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                {breakdown.y1 > 0 && breakdown.d1 > 0 && (
                  <tr>
                    <td style={tdStyle}>{breakdown.y1} yıl (1-5 yıl)</td>
                    <td style={tdRightStyle}>
                ...
                {breakdown.y2 > 0 && breakdown.d2 > 0 && (
                  <tr>
                    <td style={tdStyle}>{breakdown.y2} yıl (5-15 yıl)</td>
                    <td style={tdRightStyle}>
                ...
                {breakdown.y3 > 0 && breakdown.d3 > 0 && (
                  <tr>
                    <td style={tdStyle}>{breakdown.y3} yıl (15+ yıl)</td>
                    <td style={tdRightStyle}>
                ...
                <tr style={{ fontWeight: 600, backgroundColor: "#f3f4f6" }}>
                  ...
                  <td style={tdRightStyle}>{breakdown.total} gün</td>
                </tr>
                <tr>
                  <td style={tdStyle}>Kullanılan İzin</td>
                  <td style={tdRightStyle}>{usedTotal} gün</td>
                </tr>
                <tr style={{ fontWeight: 600, backgroundColor: "#dcfce7" }}>
                  <td style={tdStyle}>Kalan İzin</td>
                  <td style={tdRightStyle}>{remainingDays} gün</td>
                </tr>
              </tbody>
            </table>
  ...
  }, [workPeriods, diff, brutUcret, breakdown, usedTotal, remainingDays, brutIzin, sgk, issizlik, gelirVergisi, gelirVergisiDilimleri, damgaVergisi, netIzin, employerPayment, rows]);
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (belirliReportConfig.infoRows?.length) {
    ...
    sections.push({
      ...
      title: "Yıllık Ücretli İzin Hesaplama",
      html: buildWordTable(n2b.headers, n2b.rows),
    });
    ...
    return sections;
  }, [belirliReportConfig, workPeriods, breakdown, usedTotal, remainingDays, brutUcret, brutIzin, rows]);
    ...
    <div className="min-h-[calc(100vh-4rem)] w-full bg-gray-50 dark:bg-gray-900 pb-28">
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
          <div className="p-4 sm:p-6 space-y-5">
            <div className="space-y-3">
              ...
              {workPeriods.map((period, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">Dönem {index + 1}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateWorkPeriod(period.id, { iseGiris: clampYearInput(e.target.value) })}
                        onBlur={(e) => {
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateWorkPeriod(period.id, { istenCikis: clampYearInput(e.target.value) })}
                        onBlur={(e) => {
                          const newValue = e.target.value;
                          if (
                            newValue &&
              ...
              <div>
                <label className={labelCls}>Hizmet süresi (ilk giriş — son çıkış)</label>
            ...
            <div className="p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/30">
              <div className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">Yıllık izin hesaplama</div>
              ...
              <p className="mt-3 text-sm font-semibold text-gray-900 dark:text-white">
                Toplam yıllık izin hakkı: {breakdown.total} gün
              </p>
            </div>
            ...
            <div className="p-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900/20">
              <div className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">Yıllık izin hak edişi</div>
              <table className="w-full text-sm">
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                  ...
                  <tr>
                    <td className="py-2">Kullanılan izin</td>
                  ...
                  <tr className="font-semibold bg-green-100/80 dark:bg-green-900/30">
                    <td className="py-2">Kalan izin</td>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-950/25 dark:to-indigo-950/25 border-l-4 border-purple-500">
              <h3 className="text-sm font-bold text-purple-900 dark:text-purple-300 mb-3">Yıllık ücretli izin hesaplama</h3>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-l-4 border-amber-500">
              ...
              <div className="text-xs space-y-2 text-gray-800 dark:text-gray-200">
                <div className="flex justify-between">
                  <span>Brüt yıllık izin ücreti</span>
                ...
                <div className="flex justify-between pt-2 font-semibold text-green-700 dark:text-green-400">
                  <span>Net yıllık izin ücreti</span>
      ...
      <FooterActions
        ...
        previewButton={{
          ...
          copyTargetId: "yillik-belirli-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #yillik-belirli-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #yillik-belirli-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="yillik-belirli-word-copy">
          ...
          onPdf: () => downloadPdfFromDOM(`Belirli Süreli ${REPORT_TITLE} Rapor`, REPORT_CONTENT_ID),
        }}
      />
