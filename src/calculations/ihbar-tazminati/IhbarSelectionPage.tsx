...
import { Briefcase, Clock, FileCheck, Newspaper, Scale, Ship, Sun } from "lucide-react";

const cards = [
  { title: "İş Kanununa Göre", to: "/ihbar-tazminati/30isci", icon: Briefcase, color: "amber" },
  { title: "Borçlar Kanunu İşçi Alacağı", to: "/ihbar-tazminati/borclar", icon: Scale, color: "slate" },
  { title: "Gemi Adamları", to: "/ihbar-tazminati/gemi", icon: Ship, color: "sky" },
  { title: "Mevsimlik İşçi", to: "/ihbar-tazminati/mevsim", icon: Sun, color: "orange" },
  { title: "Basın İşçileri", to: "/ihbar-tazminati/basin", icon: Newspaper, color: "rose" },
  { title: "Kısmi Süreli / Part Time", to: "/ihbar-tazminati/kismi", icon: Clock, color: "violet" },
  { title: "Belirli Süreli İş Sözleşmesi", to: "/ihbar-tazminati/belirli", icon: FileCheck, color: "emerald" },
];
...
const cardStyles: Record<string, { card: string; icon: string; accent: string }> = {
  ...
  sky: {
    card: "bg-gradient-to-br from-sky-50 to-cyan-50 dark:from-sky-950/30 dark:to-cyan-950/30 ring-1 ring-sky-200/70 dark:ring-sky-800/40 group-hover:ring-sky-400/80 dark:group-hover:ring-cyan-600/50",
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <div className="w-full px-3 sm:px-[50px]">
        <div className="mb-8 sm:mb-10 text-center">
          <h1 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">İhbar Tazminatı</h1>
