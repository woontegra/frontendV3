/**
 * Gemi adamı — günlük çalışan fazla mesai. API: /api/fm/gemi.
 * Tanık yoksa davacı dönemi ve davacı saatleriyle tek tanık olarak hesaplanır.
 */
...
const REDIRECT_BASE_GUNLUK = "/fazla-mesai/gemi-adami-gunluk";
...
export default function GemiAdamiGunlukPage() {
  ...
  const recordType = RECORD_GUNLUK;
  const redirectBase = REDIRECT_BASE_GUNLUK;
  const pageTitle = "Gemi Adamı — Günlük Çalışan Fazla Mesai";
  ...
  useEffect(() => {
    ...
    let mounted = true;
    yukleHesap(effectiveId, RECORD_GUNLUK)
      .then((res) => {
        if (!mounted) return;
        if (!res.success) {
    ...
    const t = setTimeout(() => {
      ...
      (async () => {
        try {
          ...
          const witnessesValid = witnessesPayloadRaw.filter((w) => w.dateIn && w.dateOut && w.in && w.out);
          ...
          const response = await apiPost("/api/fm/gemi", payload);
    ...
    kaydetAc({
      ...
      veri: {
        data: {
          form: {
            ...formValues,
            gemiMode: "gunluk" as const,
            activeTab,
            rows,
            pageType: "gemi-adami-gunluk",
            route: redirectBase,
          },
          results: {
        ...
        formValues: { ...formValues, gemiMode: "gunluk", activeTab, rows },
  ...
  return (
    <div className={styles.workspace} data-page="fazla-mesai-gemi-adami-gunluk">
                          ...
                          {rows.length === 0
                            ? !iseGiris || !istenCikis
                              ...
                              : !normalizeTimeStr(davaci?.in) || !normalizeTimeStr(davaci?.out)
                                ? "Davacı giriş ve çıkış saatlerini girin. Tanık girmek zorunlu değildir."
