/**
 * Tanıklı Standart — UBGT / yıllık izin düşüm satırı köprüsü.
...
/** splitByExclusions tarafından işlenen; motorun işlemediği tipler → eski yol. */
const LEGACY_ONLY_EXCLUSION_TYPES = new Set(["Rapor", "Diğer", "Puantaj/Bordro"]);

export interface ExpandTanikliRowsForDeductionsParams {
...
/** Rapor / Diğer / Puantaj vb. varsa tüm liste için splitByExclusions. */
...
function expandWithMotor(
  ...
  rows.forEach((row, rowIdx) => {
    ...
    if (deductionSegments.length === 0) {
      out.push(
        ...enrichRowsWithoutDeductions([row], weeklyOffDay),
      );
      return;
    }
 ...
 */
export function expandTanikliRowsForDeductions(
  params: ExpandTanikliRowsForDeductionsParams,
): FazlaMesaiRowBase[] {
